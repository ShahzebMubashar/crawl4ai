...
def get_browser_config() -> BrowserConfig:
    ...
    return BrowserConfig(
        browser_type="chromium",  # Use Chromium instead of Firefox
...
def get_llm_strategy() -> LLMExtractionStrategy:
    ...
    return LLMExtractionStrategy(
        llm_config=LLMConfig(
            provider="gpt-4-turbo",  # Name of the LLM provider
            api_token=os.getenv("GPT_API_KEY"),  # API token for authentication
        ),
...
async def fetch_and_process_page(
    ...
    print("Fetching URL:", url)  # Add this line for debugging
